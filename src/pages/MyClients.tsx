        ...
        {filteredClients.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredClients.map((item) => {
              ...
              return (
                <Card key={item.id} className="bg-white/5 backdrop-blur-md border-white/10 hover:bg-white/10 transition-all group flex flex-col h-full">
                  <CardHeader className="pb-3">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        <Avatar className="border-2 border-white/10 h-12 w-12">
                          <AvatarImage src={client.avatar_url || ''} />
                          <AvatarFallback className="bg-slate-800 text-primary font-bold">{client.full_name?.[0] || '?'}</AvatarFallback>
                        </Avatar>
                        <div>
                          <CardTitle className="text-base text-white truncate max-w-[140px]" title={client.full_name}>{client.full_name || 'Sem nome'}</CardTitle>
                          <div className="flex items-center gap-2 mt-1">
                             <Badge variant="outline" className="h-5 px-1.5 border-yellow-500/30 text-yellow-400 flex items-center gap-1 text-[10px]"><Trophy className="h-3 w-3"/> Lvl {client.level || 1}</Badge>
                          </div>
                        </div>
                      </div>
                  ...
                  <CardContent className="flex-1 flex flex-col gap-4">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      <Badge variant="secondary" className={`text-[10px] border ${hasActiveMealPlan ? 'bg-orange-500/10 text-orange-400 border-orange-500/20' : 'bg-gray-500/10 text-gray-500 border-gray-500/20'}`}>
                          ...
                          {otherProfessionals.map((prof: any) => (
                            <Tooltip key={prof.id}>
                              <TooltipTrigger asChild>
                                <Avatar className="inline-block h-8 w-8 rounded-full ring-2 ring-slate-900 cursor-pointer border border-white/10 hover:z-10 transition-transform hover:scale-110">
                                  <AvatarImage src={prof.avatar_url || ''} />
                                  <AvatarFallback className="bg-slate-800 text-gray-400 text-[10px]">{prof.full_name?.[0] || 'P'}</AvatarFallback>
                                </Avatar>
                              </TooltipTrigger>
                              <TooltipContent className="bg-slate-800 border-white/10 text-white text-xs">
                                <p className="font-bold">{prof.full_name}</p>
                                <p className="text-primary">{translateSpecialty(prof.details?.specialty)}</p>
                              </TooltipContent>
                            </Tooltip>
                          ))}
                        </div>
                    ...
                    <Button onClick={() => navigate(`/app/clients/${client.id}`)} className="w-full mt-2 bg-white/5 hover:bg-white/10 border border-white/10 text-primary transition-colors" variant="ghost">
                      Gerenciar Aluno
                    </Button>
                  </CardContent>
                </Card>

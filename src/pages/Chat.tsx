  ...
  const fetchContacts = async () => {
    ...
    try {
      ...
      if (profile?.role === 'client') {
        ...
        // Busca conversas que o usuário tem mensagens
      ...
      const enriched = await Promise.all(contactsData.map(async (contact) => {
        ...
        // Simplificação para evitar erro de sintaxe na query or(...) complexa
        const { data: lastSent } = await supabase.from('chat_messages')
            .select('content, created_at').eq('sender_id', user.id).eq('receiver_id', contact.id)
            .order('created_at', { ascending: false }).limit(1).maybeSingle()
            
        const { data: lastReceived } = await supabase.from('chat_messages')
            .select('content, created_at').eq('sender_id', contact.id).eq('receiver_id', user.id)
            .order('created_at', { ascending: false }).limit(1).maybeSingle()
            
        let realLast = null
        if (lastSent && lastReceived) {
            realLast = new Date(lastSent.created_at) > new Date(lastReceived.created_at) ? lastSent : lastReceived
        } else {
            realLast = lastSent || lastReceived
        }

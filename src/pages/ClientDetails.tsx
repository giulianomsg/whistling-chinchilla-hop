...
import { Checkbox } from '@/components/ui/checkbox' // Certifique-se de ter este componente ou use input type="checkbox"
import { Progress } from '@/components/ui/progress'
import { 
  User, Mail, Phone, ArrowLeft, Dumbbell, Utensils, 
  Loader2, Plus,
  FileText, Save, HeartPulse, Activity, Apple, Scale, Ruler, TrendingUp,
  Pencil, Trash2, LayoutDashboard, Trophy, MessageSquare, Camera, Image as ImageIcon, Briefcase, AlertTriangle
} from 'lucide-react'
...
// Listas baseadas no PDF Anamnese-Adulto.pdf
const COMMON_CONDITIONS = ['Diabetes', 'Hipertensão', 'Asma', 'Artrite', 'Problema Renal', 'Anemia', 'Problemas Oculares', 'Obesidade']
const COMMON_SYMPTOMS = ['Dor no Peito', 'Falta de Ar', 'Tontura', 'Palpitações', 'Dores Articulares', 'Dor nas Costas', 'Fraqueza']
...
const ClientDetails: React.FC = () => {
  ...
  // Controle de Abas (Para Acesso Rápido funcionar)
  ...
  // --- FORMULÁRIO ANAMNESE COMPLETO (Baseado no PDF) ---
  const [anamnesisForm, setAnamnesisForm] = useState({
    // Identificação / Rotina
    occupation: '',
    work_hours: '',
    ...
    // Histórico Médico
    family_history: '', // Pai/Mãe/Irmão com cardiopatia
    ...
    // Comportamento
    smoker: false,
    alcohol: 'never', // never, socially, frequently
  ...
  const handleAssignWorkout = async () => {
    try {
      ...
      setActiveTab('workouts') // Redireciona para aba
  ...
  const handleAssignMealPlan = async () => {
    try {
      ...
      setActiveTab('meal-plans') // Redireciona
    } catch (err) { showError('Erro ao atribuir') }
  }
  ...
  // Helper para Checkboxes Multiplos
  ...
  return (
    <div className="min-h-screen bg-background py-4 md:py-8 w-full overflow-x-hidden">
      <div className="w-full px-4 md:max-w-7xl md:mx-auto md:px-8">
        ...
        {/* Tabs com estado controlado e style grid para fix mobile */}
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6 w-full" style={{ display: 'grid' }}>
          ...
          {/* --- DASHBOARD (LAYOUT RICO RESTAURADO) --- */}
          <TabsContent value="dashboard" className="animate-in fade-in slide-in-from-left-2 duration-500 space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full">
                ...
                <CardContent className="pt-8 px-6 md:px-10 pb-8">
                  <div className="flex flex-col md:flex-row gap-8 items-center md:items-start w-full">
                    <div className="relative flex-shrink-0">
                      <div className="w-32 h-32 rounded-full border-4 border-primary/20 p-1 mx-auto shadow-2xl">
              ...
              {/* 2. AÇÕES RÁPIDAS (Botões Verticais Grandes no Desktop, Grid no Mobile) */}
          ...
          {/* --- ANAMNESE PROFISSIONAL (Baseada no PDF) --- */}
          <TabsContent value="anamnesis">
            ...
            <Tabs defaultValue="general" className="w-full">
              <TabsList className="bg-black/20 border border-white/10 w-full justify-start h-auto flex-wrap">
                <TabsTrigger value="general" className="flex-1 h-10">Identificação</TabsTrigger>
                <TabsTrigger value="medical" className="flex-1 h-10">Clínico</TabsTrigger>
                <TabsTrigger value="habits" className="flex-1 h-10">Hábitos</TabsTrigger>
                <TabsTrigger value="nutri" className="flex-1 h-10">Nutrição</TabsTrigger>
              </TabsList>

              {/* 1. Identificação & Trabalho */}
                ...
                <Card className="bg-white/5 border-white/10">
                  <CardContent className="p-6 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div><Label className="text-gray-300 mb-2 block">Horas de Trabalho</Label><Input value={anamnesisForm.work_hours} onChange={e => updateAnamnesis('work_hours', e.target.value)} className="bg-black/20 border-white/10"/></div>
                    </div>
                    <div>
                      <Label className="text-gray-300 mb-3 block">Atividades no Trabalho</Label>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {WORK_ACTIVITIES.map(act => (
                          <div key={act} className="flex items-center space-x-2">
                            <input type="checkbox" checked={anamnesisForm.work_activities?.includes(act)} onChange={() => toggleAnamnesisList('work_activities', act)} className="rounded border-white/20 bg-black/40"/>
              ...
              {/* 2. Histórico Clínico (Expandido) */}
              <TabsContent value="medical">
                <Card className="bg-white/5 border-white/10">
                  <CardContent className="p-6 space-y-6">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                       {COMMON_CONDITIONS.map(cond => (
                          <div key={cond} className="flex items-center space-x-2 p-3 rounded bg-black/20 border border-white/5">
                            <input type="checkbox" checked={anamnesisForm.diagnosed_conditions?.includes(cond)} onChange={() => toggleAnamnesisList('diagnosed_conditions', cond)}/>
                            <span className="text-xs font-bold text-gray-300">{cond}</span>
                          </div>
                       ))}
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div><Label className="text-gray-300 mb-2 block">Histórico Familiar (Cardiopatias)</Label><Textarea value={anamnesisForm.family_history} onChange={e => updateAnamnesis('family_history', e.target.value)} className="bg-black/20 border-white/10 min-h-[80px]"/></div>
                      <div><Label className="text-gray-300 mb-2 block">Cirurgias</Label><Textarea value={anamnesisForm.surgeries} onChange={e => updateAnamnesis('surgeries', e.target.value)} className="bg-black/20 border-white/10 min-h-[80px]"/></div>
                      <div><Label className="text-gray-300 mb-2 block">Lesões / Dores</Label><Textarea value={anamnesisForm.injuries} onChange={e => updateAnamnesis('injuries', e.target.value)} className="bg-black/20 border-white/10 min-h-[80px]"/></div>
                      <div><Label className="text-gray-300 mb-2 block">Medicamentos</Label><Textarea value={anamnesisForm.medications} onChange={e => updateAnamnesis('medications', e.target.value)} className="bg-black/20 border-white/10 min-h-[80px]"/></div>
                    </div>
                    <div>
                      <Label className="text-gray-300 mb-3 block">Sintomas Frequentes</Label>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {COMMON_SYMPTOMS.map(sym => (
                          <div key={sym} className="flex items-center space-x-2">
                            <input type="checkbox" checked={anamnesisForm.symptoms?.includes(sym)} onChange={() => toggleAnamnesisList('symptoms', sym)} className="rounded border-white/20 bg-black/40"/>
                            <span className="text-sm text-gray-400">{sym}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <CardContent className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <Select value={anamnesisForm.alcohol} onValueChange={v => updateAnamnesis('alcohol', v)}>
                        ...
                        <SelectContent className="bg-slate-800 border-white/10 text-white"><SelectItem value="never">Nunca</SelectItem><SelectItem value="socially">Socialmente</SelectItem><SelectItem value="frequently">Frequentemente</SelectItem></SelectContent>
                      </Select>
                    </div>
                    <div className="md:col-span-2"><Label className="text-gray-300 mb-2 block">Restrições Físicas</Label><Textarea value={anamnesisForm.physical_restrictions} onChange={e => updateAnamnesis('physical_restrictions', e.target.value)} className="bg-black/20 border-white/10 min-h-[80px]"/></div>
                  </CardContent>
                </Card>
              </TabsContent>
              ...
              <TabsContent value="nutri">
                 <Card className="bg-white/5 border-white/10">
                    <CardContent className="p-6 space-y-6">
                       ...
                       <div><Label className="text-gray-300 mb-2 block">Histórico Alimentar / Aversões</Label><Textarea value={anamnesisForm.diet_history} onChange={e => updateAnamnesis('diet_history', e.target.value)} className="bg-black/20 border-white/10 min-h-[120px]"/></div>
                    </CardContent>
                 </Card>
              </TabsContent>
            </Tabs>
          ...
          <TabsContent value="photos">
            <Card className="bg-white/5 border-white/10 w-full">
                  ...
                  <Dialog open={isAddPhotoOpen} onOpenChange={setIsAddPhotoOpen}>
                    ...
                    <DialogContent className="bg-slate-900 border-white/10 text-white w-[95%] rounded-lg">
                       ...
                       <div className="space-y-4 mt-4">
                          ...
                          <div><Label>Notas</Label><Input placeholder="Ex: Frente" value={newPhoto.notes} onChange={e => setNewPhoto({...newPhoto, notes: e.target.value})} className="bg-black/20 border-white/10 text-white"/></div>
               ...
               <CardContent className="p-6">
                  {progressPhotos.length === 0 ? <div className="text-center py-12 text-gray-500">Nenhuma foto.</div> : (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                       {progressPhotos.map(photo => (
                         <div key={photo.id} className="group relative bg-black/40 rounded-xl overflow-hidden border border-white/10 aspect-[3/4] shadow-lg">
                            ...
                            <div className="absolute bottom-0 left-0 right-0 bg-black/60 p-2 text-center md:hidden backdrop-blur-sm"><span className="text-xs font-bold text-white">{new Date(photo.date).toLocaleDateString('pt-BR')}</span></div>
                         </div>
                       ))}
                    </div>
                  )}
          ...
          <TabsContent value="biometrics">
            <Card className="bg-white/5 border-white/10 w-full">
              ...
              <CardContent className="p-6">
                {assessments.length === 0 ? <div className="text-center text-gray-500 py-12">Nenhuma avaliação.</div> : (
                  <div className="space-y-4">
                    {assessments.map((a) => {
                      const s = a.measurements?.status || 'completed'
                      const c = a.measurements?.completion || 0
                        ...
                        <div key={a.id} className={`bg-black/20 p-5 rounded-xl border ${s==='draft' ? 'border-yellow-500/30' : 'border-white/5'} flex flex-col md:flex-row justify-between items-start md:items-center gap-4`}>
                          <div className="flex items-center gap-5 w-full md:w-auto">
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-3"><span className="text-white font-bold text-xl">{a.weight} kg</span>{s==='draft'?<Badge variant="secondary" className="text-yellow-400 bg-yellow-900/20 border-none text-xs">Rascunho</Badge>:<Badge variant="outline" className={`text-xs ${i.color} border-current`}>{i.label}</Badge>}</div>
                              {s==='draft'?(<div className="w-full md:w-40 mt-2"><Progress value={c} className="h-1.5 bg-white/10"/></div>):(<div className="text-sm text-gray-400 flex gap-4 mt-1"><span>Gord: {a.body_fat_percentage}%</span><span>Massa: {a.muscle_mass}kg</span></div>)}
                            </div>
                          </div>
                          <div className="flex gap-2 w-full md:w-auto justify-end pt-4 md:pt-0"><Button variant="ghost" size="sm" onClick={()=>openEditAssessment(a)} className="text-blue-400 hover:bg-blue-500/10"><Pencil className="h-4 w-4"/></Button><Button variant="ghost" size="sm" onClick={()=>handleDeleteAssessment(a.id)} className="text-red-400 hover:bg-red-500/10"><Trash2 className="h-4 w-4"/></Button></div>
                        </div>
                      )
                    })}
                  </div>
          ...
          {/* ABAS SECUNDÁRIAS (TREINOS, DIETAS, HISTÓRICO, PERFIL) - Mantidas para garantir integridade */}

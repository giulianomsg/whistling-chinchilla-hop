  ...
  return (
    <div className="min-h-screen bg-background py-4 md:py-8">
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Tabs Container - Controla o overflow horizontal */}
        <Tabs defaultValue="dashboard" className="space-y-6 w-full">
          <div className="w-full overflow-x-auto pb-2">
            <TabsList className="bg-white/5 border border-white/10 justify-start p-1 flex w-max md:w-auto">
              <TabsTrigger value="dashboard" className="px-4 py-2 text-sm"><LayoutDashboard className="w-4 h-4 mr-2"/> Visão Geral</TabsTrigger>
              <TabsTrigger value="workouts" className="px-4 py-2 text-sm">Treinos</TabsTrigger>
              <TabsTrigger value="meal-plans" className="px-4 py-2 text-sm">Dietas</TabsTrigger>
              <TabsTrigger value="biometrics" className="px-4 py-2 text-sm"><Scale className="w-4 h-4 mr-2"/> Biometria</TabsTrigger>
              <TabsTrigger value="anamnesis" className="px-4 py-2 text-sm"><FileText className="w-4 h-4 mr-2"/> Anamnese</TabsTrigger>
              <TabsTrigger value="history" className="px-4 py-2 text-sm">Histórico</TabsTrigger>
              <TabsTrigger value="info" className="px-4 py-2 text-sm">Perfil</TabsTrigger>
            </TabsList>
          </div>

          {/* --- DASHBOARD --- */}
          <TabsContent value="dashboard" className="animate-in fade-in slide-in-from-left-2 duration-500">
            {/* Grid principal: 1 coluna no mobile (vertical), 3 colunas no desktop (horizontal) */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full">
                ...
                <CardContent className="pt-8 px-6 pb-8">
                  <div className="flex flex-col md:flex-row gap-6 items-center md:items-start">
                    ...
                    <div className="flex-1 w-full text-center md:text-left min-w-0">
                      <h2 className="text-2xl md:text-4xl font-bold text-white mb-2 truncate">{clientProfile?.full_name}</h2>
                      <div className="flex flex-col md:flex-row items-center md:justify-start gap-2 md:gap-6 text-sm text-gray-400 mb-6">
                        <span className="flex items-center gap-2 truncate"><Mail className="w-4 h-4 text-primary flex-shrink-0"/> {clientProfile?.email}</span>
                        {clientProfile?.phone && <span className="flex items-center gap-2 truncate"><Phone className="w-4 h-4 text-primary flex-shrink-0"/> {clientProfile?.phone}</span>}
                      </div>
                      ...
                      <div className="space-y-2 w-full max-w-md mx-auto md:mx-0">
                        ...
                        <p className="text-xs text-gray-500 mt-1 text-right">Próximo nível em {1000 - (currentXP % 1000)} XP</p>
                      </div>
                    </div>
                  </div>
                </CardContent>
              ...
              <Card className="bg-white/5 border-white/10 h-full">
                ...
                <CardContent className="grid grid-cols-2 gap-4">
                  <Button variant="outline" className="h-24 flex-col border-white/10 bg-white/5 hover:bg-white/10 text-white gap-2" onClick={() => setIsAssignWorkoutOpen(true)}>
                    <Dumbbell className="h-8 w-8 text-blue-400 mb-1"/> <span className="text-xs font-medium">Atribuir Treino</span>
                  </Button>
                  <Button variant="outline" className="h-24 flex-col border-white/10 bg-white/5 hover:bg-white/10 text-white gap-2" onClick={() => setIsAssignMealPlanOpen(true)}>
                    <Utensils className="h-8 w-8 text-orange-400 mb-1"/> <span className="text-xs font-medium">Atribuir Dieta</span>
                  </Button>
                  <Button variant="outline" className="h-24 flex-col border-white/10 bg-white/5 hover:bg-white/10 text-white gap-2" onClick={openNewAssessment}>
                    <Scale className="h-8 w-8 text-green-400 mb-1"/> <span className="text-xs font-medium">Nova Avaliação</span>
                  </Button>
                  <Button variant="outline" className="h-24 flex-col border-white/10 bg-white/5 hover:bg-white/10 text-white gap-2" onClick={() => navigate('/app/chat')}>
                    <MessageSquare className="h-8 w-8 text-purple-400 mb-1"/> <span className="text-xs font-medium">Mensagem</span>
                  </Button>
                </CardContent>
              </Card>

              {/* 3. Métricas - 1 coluna no mobile, 4 no desktop */}
          ...
          {/* Abas Secundárias - Preservando Layout Desktop Rico */}
            ...
            <Card className="bg-white/5 border-white/10">
              <CardHeader className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6">
                ...
                <Dialog open={isNewAssessmentOpen} onOpenChange={setIsNewAssessmentOpen}>
                  <DialogTrigger asChild><Button onClick={openNewAssessment} className="bg-primary text-black hover:bg-primary/80 font-bold w-full md:w-auto"><Plus className="w-4 h-4 mr-2"/> Nova Avaliação</Button></DialogTrigger>
                  <DialogContent className="bg-slate-900 border-white/10 text-white w-full max-w-4xl max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 py-4">
                      ...
                      <div className="space-y-4"><h3 className="font-semibold text-primary">Dobras</h3><div className="grid grid-cols-2 gap-2">{Object.keys(newAssessment.skinfolds).map(k => (<div key={k}><Label className="text-xs text-gray-400 uppercase">{SKINFOLD_LABELS[k]?.slice(0,3)}</Label><Input type="number" className="h-8 bg-black/20 border-white/10 text-white" value={(newAssessment.skinfolds as any)[k]} onChange={e => updateNested('skinfolds', k, e.target.value)}/></div>))}</div></div>
                      <div className="space-y-4"><h3 className="font-semibold text-primary">Perímetros</h3><div className="grid grid-cols-2 gap-2">{Object.keys(newAssessment.circumferences).map(k => (<div key={k}><Label className="text-xs text-gray-400 uppercase">{CIRCUMFERENCE_LABELS[k]?.slice(0,3)}</Label><Input type="number" className="h-8 bg-black/20 border-white/10 text-white" value={(newAssessment.circumferences as any)[k]} onChange={e => updateNested('circumferences', k, e.target.value)}/></div>))}</div></div>
                    </div>
                    <DialogFooter className="flex-col md:flex-row gap-2"><Button variant="outline" onClick={() => handleSaveAssessment('draft')} className="w-full md:w-auto border-white/10 text-white hover:bg-white/5">Salvar Rascunho</Button><Button onClick={() => handleSaveAssessment('completed')} className="w-full md:w-auto bg-green-600 text-white hover:bg-green-700">Finalizar</Button></DialogFooter>
                  </DialogContent>
                </Dialog>
              </CardHeader>
                ...
                {assessments.length === 0 ? <div className="text-center text-gray-500 py-12">Nenhuma avaliação registrada.</div> : (
                  <div className="space-y-4">
                    {assessments.map((assessment) => {
                      ...
                      return (
                        <div key={assessment.id} className={`bg-black/20 p-4 md:p-5 rounded-xl border ${status === 'draft' ? 'border-yellow-500/30 bg-yellow-900/5' : 'border-white/5'} flex flex-col md:flex-row justify-between items-start md:items-center gap-4`}>
                          <div className="flex items-center gap-5 w-full md:w-auto">
                            <div className={`w-14 h-14 rounded-full flex items-center justify-center font-bold text-sm flex-col flex-shrink-0 ${status === 'draft' ? 'bg-yellow-500/10 text-yellow-500' : 'bg-primary/10 text-primary'}`}><span>{new Date(assessment.date).getDate()}</span><span className="uppercase text-[10px]">{new Date(assessment.date).toLocaleString('default', { month: 'short' })}</span></div>
                            <div className="flex-1">
                              <div className="flex items-center gap-3"><span className="text-white font-bold text-xl">{assessment.weight} kg</span>{status === 'draft' ? <Badge variant="secondary" className="text-yellow-400 bg-yellow-900/20 border-none text-xs">Rascunho</Badge> : <Badge variant="outline" className={`text-xs ${bmiInfo.color} border-current`}>{bmiInfo.label}</Badge>}</div>
                              {status === 'draft' ? (<div className="w-full md:w-40 mt-2"><div className="flex justify-between text-xs text-gray-400 mb-1"><span>Preenchimento</span><span>{completion}%</span></div><Progress value={completion} className="h-1.5 bg-white/10" /></div>) : (<div className="text-sm text-gray-400 flex gap-4 mt-1"><span>Gordura: {assessment.body_fat_percentage}%</span><span className="hidden sm:inline">•</span><span>Massa: {assessment.muscle_mass}kg</span></div>)}
                            </div>
                          </div>
                          <div className="flex gap-2 w-full md:w-auto justify-end border-t md:border-t-0 border-white/10 pt-4 md:pt-0"><Button variant="ghost" size="sm" onClick={() => openEditAssessment(assessment)} className="text-blue-400 hover:bg-blue-500/10"><Pencil className="h-4 w-4 mr-2 md:mr-0"/><span className="md:hidden">Editar</span></Button><Button variant="ghost" size="sm" onClick={() => handleDeleteAssessment(assessment.id)} className="text-red-400 hover:bg-red-500/10"><Trash2 className="h-4 w-4 mr-2 md:mr-0"/><span className="md:hidden">Excluir</span></Button></div>
                        </div>
                      )
                    })}
                  </div>
          ...
          <TabsContent value="workouts"><Card className="bg-white/5 border-white/10"><CardHeader className="p-6 flex flex-col md:flex-row items-center justify-between gap-4"><CardTitle className="text-white text-lg">Treinos</CardTitle><Dialog open={isAssignWorkoutOpen} onOpenChange={setIsAssignWorkoutOpen}><DialogTrigger asChild><Button size="sm" className="bg-blue-600 w-full md:w-auto"><Plus className="mr-2 h-4 w-4"/> Atribuir</Button></DialogTrigger><DialogContent className="bg-slate-900 border-white/10 text-white w-[95%]"><DialogHeader><DialogTitle>Atribuir</DialogTitle></DialogHeader><div className="space-y-4 mt-4"><Select onValueChange={setSelectedWorkoutId}><SelectTrigger className="bg-black/20 border-white/10"><SelectValue placeholder="Treino..."/></SelectTrigger><SelectContent className="bg-slate-800 border-white/10 text-white">{availableWorkouts.map(w => <SelectItem key={w.id} value={w.id}>{w.name}</SelectItem>)}</SelectContent></Select><Input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="bg-black/20 border-white/10 text-white"/><Button onClick={handleAssignWorkout} className="w-full bg-blue-600 hover:bg-blue-500">Confirmar</Button></div></DialogContent></Dialog></CardHeader><CardContent className="p-6">{clientWorkouts.map(cw => (<div key={cw.id} className="bg-black/20 p-4 rounded-lg border border-white/5 mb-3 flex flex-col md:flex-row justify-between items-center gap-3"><div><h4 className="text-sm font-bold text-white text-center md:text-left">{cw.workout.name}</h4><div className="text-xs text-gray-400 text-center md:text-left">{cw.workout.days_per_week}x semana</div></div><Button size="sm" variant="ghost" onClick={() => handleRemoveAssignment('client_workouts', cw.id)} className="text-red-400 hover:bg-red-900/20 w-full md:w-auto"><Trash2 className="h-4 w-4 mr-2"/> Remover</Button></div>))}</CardContent></Card></TabsContent>
          
          <TabsContent value="meal-plans"><Card className="bg-white/5 border-white/10"><CardHeader className="p-6 flex flex-col md:flex-row items-center justify-between gap-4"><CardTitle className="text-white text-lg">Dietas</CardTitle><Dialog open={isAssignMealPlanOpen} onOpenChange={setIsAssignMealPlanOpen}><DialogTrigger asChild><Button size="sm" className="bg-green-600 w-full md:w-auto"><Plus className="mr-2 h-4 w-4"/> Atribuir</Button></DialogTrigger><DialogContent className="bg-slate-900 border-white/10 text-white w-[95%]"><DialogHeader><DialogTitle>Atribuir</DialogTitle></DialogHeader><div className="space-y-4 mt-4"><Select onValueChange={setSelectedMealPlanId}><SelectTrigger className="bg-black/20 border-white/10"><SelectValue placeholder="Dieta..."/></SelectTrigger><SelectContent className="bg-slate-800 border-white/10 text-white">{availableMealPlans.map(p => <SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>)}</SelectContent></Select><Input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="bg-black/20 border-white/10 text-white"/><Button onClick={handleAssignMealPlan} className="w-full bg-green-600 hover:bg-green-500">Confirmar</Button></div></DialogContent></Dialog></CardHeader><CardContent className="p-6">{clientMealPlans.map(cm => (<div key={cm.id} className="bg-black/20 p-4 rounded-lg border border-white/5 mb-3 flex flex-col md:flex-row justify-between items-center gap-3"><div><h4 className="text-sm font-bold text-white text-center md:text-left">{cm.meal_plan.name}</h4><div className="text-xs text-gray-400 text-center md:text-left">{cm.meal_plan.daily_calories_target} kcal</div></div><Button size="sm" variant="ghost" onClick={() => handleRemoveAssignment('client_meal_plans', cm.id)} className="text-red-400 hover:bg-red-900/20 w-full md:w-auto"><Trash2 className="h-4 w-4 mr-2"/> Remover</Button></div>))}</CardContent></Card></TabsContent>
          
          <TabsContent value="anamnesis"><div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4"><h2 className="text-2xl font-bold text-white flex items-center gap-2"><FileText className="text-primary"/> Anamnese Profissional</h2><Button onClick={handleSaveAnamnesis} className="bg-primary text-black hover:bg-primary/80 font-bold shadow-lg w-full md:w-auto"><Save className="mr-2 h-4 w-4"/> Salvar Ficha</Button></div><Tabs defaultValue="medical" className="w-full"><TabsList className="bg-black/20 border border-white/10 w-full justify-start h-12 mb-6"><TabsTrigger value="medical" className="flex-1 h-10">Saúde</TabsTrigger><TabsTrigger value="lifestyle" className="flex-1 h-10">Estilo</TabsTrigger><TabsTrigger value="nutri" className="flex-1 h-10">Nutri</TabsTrigger></TabsList><TabsContent value="medical"><Card className="bg-white/5 border-white/10"><CardContent className="p-6 space-y-6"><div><Label className="text-gray-300 mb-2 block">Patologias</Label><Textarea value={anamnesisForm.medical_history} onChange={e => updateAnamnesis('medical_history', e.target.value)} className="bg-black/20 border-white/10 min-h-[100px]"/></div><div><Label className="text-gray-300 mb-2 block">Lesões</Label><Textarea value={anamnesisForm.injuries} onChange={e => updateAnamnesis('injuries', e.target.value)} className="bg-black/20 border-white/10 min-h-[100px]"/></div></CardContent></Card></TabsContent><TabsContent value="lifestyle"><Card className="bg-white/5 border-white/10"><CardContent className="p-6 space-y-6"><div><Label className="text-gray-300 mb-2 block">Profissão</Label><Input value={anamnesisForm.occupation} onChange={e => updateAnamnesis('occupation', e.target.value)} className="bg-black/20 border-white/10 h-10"/></div><div className="flex justify-between items-center bg-black/20 p-4 rounded border border-white/5"><Label className="text-gray-300">Fumante?</Label><Switch checked={anamnesisForm.smoker} onCheckedChange={c => updateAnamnesis('smoker', c)} /></div></CardContent></Card></TabsContent><TabsContent value="nutri"><Card className="bg-white/5 border-white/10"><CardContent className="p-6 space-y-6"><div><Label className="text-gray-300 mb-2 block">Água (L)</Label><Input value={anamnesisForm.water_intake} onChange={e => updateAnamnesis('water_intake', e.target.value)} className="bg-black/20 border-white/10 h-10"/></div><div><Label className="text-gray-300 mb-2 block">Histórico</Label><Textarea value={anamnesisForm.diet_history} onChange={e => updateAnamnesis('diet_history', e.target.value)} className="bg-black/20 border-white/10 min-h-[100px]"/></div></CardContent></Card></TabsContent></Tabs></TabsContent>
